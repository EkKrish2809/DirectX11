...
public:
    ...
    ModelAnimation ourModel;
    Animation model_animation;
    Animator model_animator;

    SkinningMain() {}

    SkinningMain(IDXGISwapChain * SwapChain,
            ID3D11Device *Device,
    ...
    BOOL Initialize()
    {
        ...
        model_animation.InitializeAnimation("./assets/Models/WalkingLeftTurn.fbx", &ourModel);
        model_animator.InitializeAnimator(&model_animation);
    ...
    void RenderFrame(float time, XMMATRIX viewMat)
    {
        ...
        XMMATRIX worldMatrix = XMMatrixTranslation(0.0f, -14.0f, 125.0f);
        XMMATRIX viewMatrix = (XMMatrixLookAtLH(Eye, At, Up));
        XMMATRIX scaleMatrix = (XMMatrixScaling(.1f, .1f, .1f));
