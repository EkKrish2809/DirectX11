...
bool ReflectionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"./src/Shaders/Reflection/reflection_vs.hlsl");
    if(error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"./src/Shaders/Reflection/reflection_ps.hlsl");
    if(error != 0)
    {
        return false;
    }
    ...
    result = InitializeShader(device, NULL, vsFilename, psFilename);
    if(!result)
    {
        return false;
    }
...
bool ReflectionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
    ...
    if(FAILED(result))
    {
        ...
        if(errorMessage)
        {
            ...
            fprintf(gpFile, "Vertex Shader Failed : %s\n", (char*)(errorMessage->GetBufferPointer()));
            fclose(gpFile);
        }
        ...
        else
        {
            ...
            fopen_s(&gpFile, gszLogFilePathName, "a+");
            fprintf(gpFile, "Vertex Shader : Missing Shader File \n");
            fclose(gpFile);
        }
    ...
    if(FAILED(result))
    {
        ...
        if(errorMessage)
        {
            ...
            fopen_s(&gpFile, gszLogFilePathName, "a+");
            fprintf(gpFile, "Pixel Shader Failed : %s\n", (char*)(errorMessage->GetBufferPointer()));
            fclose(gpFile);
        }
        ...
        else
        {
            ...
            fopen_s(&gpFile, gszLogFilePathName, "a+");
            fprintf(gpFile, "Pixel Shader : Missing Shader File \n");
            fclose(gpFile);
        }
