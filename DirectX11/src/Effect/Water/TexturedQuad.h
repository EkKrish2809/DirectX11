...
public:
    ...
    TexturedQuad(IDXGISwapChain *SwapChain,
              ID3D11Device *Device,
    ...
    BOOL Initialize(void)
    {
        ...
        if (shader->CreateAndCompilePixelShaderObjects("./src/Shaders/Water/Quad_PS.hlsl", PIXEL_SHADER, &gpID3D11PixelShader, &pID3DBlob_PixelShaderCode) == FALSE)
        {
            fopen_s(&gpFile, gszLogFilePathName, "a+");
        ...
        ZeroMemory((void *)&d3d11BufferDescriptor, sizeof(D3D11_BUFFER_DESC));
        d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
        d3d11BufferDescriptor.ByteWidth = sizeof(unsigned long) * _ARRAYSIZE(squareTexcoords);
        ...
        ZeroMemory((void *)&d3d11SubresourceData, sizeof(D3D11_SUBRESOURCE_DATA));
        d3d11SubresourceData.pSysMem = new unsigned long[sizeof(squareTexcoords)];
        d3d11SubresourceData.SysMemPitch = 0;
        d3d11SubresourceData.SysMemSlicePitch = 0;

        // Create the index buffer.
        hr = m_Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &pIndexBuffer);
        if(FAILED(hr))
        {
            return false;
        }
        ...
        ZeroMemory((void *)&d3d11BufferDescriptor, sizeof(D3D11_BUFFER_DESC));
        d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
        d3d11BufferDescriptor.ByteWidth = sizeof(CBUFFER_DEBUGQUAD);
    ...
    void RenderObject(Camera camera, ID3D11ShaderResourceView *fbo_texture)
    {
        // code
        ...
        CBUFFER_DEBUGQUAD constBuffer;
        ZeroMemory((void *)&constBuffer, sizeof(CBUFFER_DEBUGQUAD));
        ...
        m_DeviceContext->PSSetShaderResources(0, 1, &fbo_texture);
    ...
    void RenderBuffers(ID3D11DeviceContext* deviceContext)
    {
        ...
        deviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_PositionBuffer, &stride, &offset);

        stride = sizeof(float) * 2;
        offset = 0;
        deviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_TextureBuffer, &stride, &offset);
