...
bool ReflectionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./src/Shaders/Water/reflection.vs", L"./src/Shaders/Water/reflection.ps");
	if(!result)
	{
		return false;
	}
...
bool ReflectionShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
				   DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* colorTexture, ID3D11ShaderResourceView* normalTexture, 
				   DirectX::XMVECTOR lightDiffuseColor, DirectX::XMFLOAT3 lightDirection, float colorTextureBrightness, DirectX::XMVECTOR clipPlane)
{
	bool result;
...
bool ReflectionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// fprintf(gpFile, "shader compilation started...\n");
	// fclose(gpFile);


	// Compile the vertex shader code.
    // if (shader->CreateAndCompileVertexShaderObjects("./src/Shaders/Water/reflection.vs", VERTEX_SHADER, &m_vertexShader, &vertexShaderBuffer) == FALSE)
    // {
    //     fopen_s(&gpFile, gszLogFilePathName, "a+");
	...
	// {
	// 	fopen_s(&gpFile, gszLogFilePathName, "a+");
    //     fprintf(gpFile, "shader->CreateAndCompileShaderObjects() Succeded for Vertex Shader...\n");
    //     fclose(gpFile);
	// }

	// fopen_s(&gpFile, gszLogFilePathName, "a+");
	// fprintf(gpFile, "shader compilation Done for Vertex Shader...\n");
	// fclose(gpFile);
	...
	if(FAILED(result))
	{
		...
		{
			fopen_s(&gpFile, gszLogFilePathName, "a+");
			fprintf(gpFile, "Missing Shader File...\n");
			fclose(gpFile);
    ...
    // if (!shader->CreateAndCompilePixelShaderObjects("./src/Shaders/Water/reflection.ps", PIXEL_SHADER, &m_pixelShader, &pixelShaderBuffer))
    // {
    //     fopen_s(&gpFile, gszLogFilePathName, "a+");
	...
	// {
	// 	fopen_s(&gpFile, gszLogFilePathName, "a+");
    //     fprintf(gpFile, "shader->CreateAndCompileShaderObjects() Succeded for Pixel Shader...\n");
    //     fclose(gpFile);
	// }
...
bool ReflectionShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
						DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* colorTexture, 
						ID3D11ShaderResourceView* normalTexture, DirectX::XMVECTOR lightDiffuseColor, DirectX::XMFLOAT3 lightDirection, 
						float colorTextureBrightness, DirectX::XMVECTOR clipPlane)
{
	HRESULT result;
