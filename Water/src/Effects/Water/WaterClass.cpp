...
bool WaterClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, WCHAR* textureFilename, float waterHeight, float waterRadius)
{
	bool result;
	...
	if(!result)
	{
		...
		fprintf(gpFile, "InitializeBuffers() Failed .......\n");
		fclose(gpFile);
		return false;
	}

    /* 
        REPLACE BELOW TEXTURE LOADING CODE WITH MY TEXTURE LOADING FUNCTION
    */
	...
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "LoadTexture() Failed .......\n");
		fclose(gpFile);
		return false;
	}
	...
	m_refractionTint = DirectX::XMVectorSet(0.0f, 0.8f, 1.0f, 1.0f);
	// m_refractionTint = DirectX::XMVectorSet(0.3f, 0.8f, 1.0f, 1.0f);
...
DirectX::XMFLOAT2 WaterClass::GetNormalMapTiling()
{
...
DirectX::XMVECTOR WaterClass::GetRefractionTint()
{
...
bool WaterClass::InitializeBuffers(ID3D11Device* device, float waterRadius)
{
	...
	vertices[0].position = DirectX::XMFLOAT3(-waterRadius, 0.0f, waterRadius);  // Top left.
	vertices[0].texture = DirectX::XMFLOAT2(0.0f, 0.0f);

	vertices[1].position = DirectX::XMFLOAT3(waterRadius, 0.0f, waterRadius);  // Top right.
	vertices[1].texture = DirectX::XMFLOAT2(1.0f, 0.0f);
	
	vertices[2].position = DirectX::XMFLOAT3(-waterRadius, 0.0f, -waterRadius);  // Bottom left.
	vertices[2].texture = DirectX::XMFLOAT2(0.0f, 1.0f);

	vertices[3].position = DirectX::XMFLOAT3(-waterRadius, 0.0f, -waterRadius);  // Bottom left.
	vertices[3].texture = DirectX::XMFLOAT2(0.0f, 1.0f);

	vertices[4].position = DirectX::XMFLOAT3(waterRadius, 0.0f, waterRadius);  // Top right.
	vertices[4].texture = DirectX::XMFLOAT2(1.0f, 0.0f);

	vertices[5].position = DirectX::XMFLOAT3(waterRadius, 0.0f, -waterRadius);  // Bottom right.
...
bool WaterClass::LoadTexture(ID3D11Device* device,ID3D11DeviceContext* deviceContext, WCHAR* filename)
{
	bool result;
