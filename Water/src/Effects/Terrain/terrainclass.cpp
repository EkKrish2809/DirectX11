...
bool TerrainClass::Initialize(ID3D11Device* device, ID3D11DeviceContext *deviceContext, char* heightMapFilename, char* colorMapFilename, float flattenAmount, WCHAR* colorTextureFilename, 
							  WCHAR* normalTextureFilename)
{
	...
	if(!result)
	{
		...
		fprintf(gpFile, "LoadHeightMap failed ...\n");
		fclose(gpFile);
		return false;
	}
	...
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "LoadColorMap failed ...\n");
		fclose(gpFile);
		return false;
	}
	...
	// Load the textures.
	result = LoadTextures(device, deviceContext, colorTextureFilename, normalTextureFilename);
	if(!result)
	{
		return false;
	}
...
bool TerrainClass::LoadHeightMap(char* filename)
{
	...
	// error = fopen_s(&filePtr, "./assets/textures/hm.bmp", "rb");
	if(error != 0)
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "fopen_s() failed for Opening height map ...\n");
		fclose(gpFile);
		return false;
	}
	...
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "fread() failed for reading height map File header ...\n");
		fclose(gpFile);
		return false;
	}
	...
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "fread() failed for reading height map Info header ...\n");
		fclose(gpFile);
		return false;
	}
...
bool TerrainClass::LoadColorMap(char* filename)
{
	...
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "fopen_s() failed for Opening color map ...\n");
		fclose(gpFile);
		return false;
	}
	...
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "fread() failed for reading color map File header ...\n");
		fclose(gpFile);
		return false;
	}
	...
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "fread() failed for reading color map Info header ...\n");
		fclose(gpFile);
		return false;
	}
...
bool TerrainClass::InitializeBuffers(ID3D11Device* device)
{
	...
	for(i=0; i<m_vertexCount; i++)
	{
		...
		vertices[i].tangent  = DirectX::XMFLOAT3(m_model[i].tx, m_model[i].ty, m_model[i].tz);
		vertices[i].binormal = DirectX::XMFLOAT3(m_model[i].bx, m_model[i].by, m_model[i].bz);
		vertices[i].color    = DirectX::XMVectorSet(m_model[i].r, m_model[i].g, m_model[i].b, 1.0f);
...
bool TerrainClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext * deviceContext, WCHAR* colorTexturefilename, WCHAR* normalTextureFilename)
{
	bool result;
	...
	result = m_ColorTexture->Initialize(device, deviceContext, colorTexturefilename);
	if(!result)
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "ColorTexture Initialization failed ...\n");
		fclose(gpFile);
		return false;
	}
	...
	result = m_NormalTexture->Initialize(device, deviceContext, normalTextureFilename);
	if(!result)
	{
		fopen_s(&gpFile, gszLogFilePathName, "a+"); // opening file in append mode
		fprintf(gpFile, "NormalTexture Initialization failed ...\n");
		fclose(gpFile);
		return false;
	}
