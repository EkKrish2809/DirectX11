...
bool TerrainShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./src/Shaders/Terrain/terrain.vs", L"./src/Shaders/Terrain/terrain.ps");
	if(!result)
	{
		return false;
	}
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
								DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* colorTexture, ID3D11ShaderResourceView* normalTexture, 
								DirectX::XMVECTOR lightDiffuseColor, DirectX::XMFLOAT3 lightDirection, float colorTextureBrightness)
{
	bool result;
...
bool TerrainShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if(FAILED(result))
	{
		...
		else
		{
			...
			fprintf(gpFile, "Missing Vertex Shader File...\n");
			fclose(gpFile);
	...
	if(FAILED(result))
	{
		...
		{
			fopen_s(&gpFile, gszLogFilePathName, "a+");
			fprintf(gpFile, "Missing Pixel Shader File...\n");
			fclose(gpFile);
...
bool TerrainShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix, 
											 DirectX::XMMATRIX projectionMatrix, ID3D11ShaderResourceView* colorTexture, ID3D11ShaderResourceView* normalTexture, 
											 DirectX::XMVECTOR lightDiffuseColor, DirectX::XMFLOAT3 lightDirection, float colorTextureBrightness)
{
	HRESULT result;
